...
The temptations to regard the visible and the visibly colored as dissective are due to overlooking the partial nature of perception \citet[see][]{Hilbert:1987jq}. If the visible is dissective, then we see an object only if we see all of its parts. And if the visibly colored is dissective, then we see an object to be colored only if we see all of its parts to be that color. However, if perception provides only a partial perspective on the material environment, then each of these principles are implausible. To claim that perception is partial is to claim, in part, that seeing something does not require seeing the whole of it.
